...
" Ruby {{{
function! s:RequireRubyHost(name)
  " Ruby host arguments
  let args = ['-v', '-l', '/tmp/neovim-ruby-plugin.log']

  " Collect registered ruby plugins into args
  let ruby_plugins = s:PluginsForHost(a:name)
  for plugin in ruby_plugins
    call add(args, plugin.path)
  endfor

  " Try loading a ruby host using `ruby_host_prog` or
  " `start-neovim-ruby-plugin-host`
  let ruby_host_prog = get(g:, 'ruby_host_prog', 'start-neovim-ruby-plugin-host')

  let channel_id = rpcstart(ruby_host_prog, args)
  if rpcrequest(channel_id, 'poll') == 'ok'
    echo rpcrequest(channel_id, 'poll')
    echo rpcrequest(channel_id, 'boom')
    return channel_id
  endif

  throw 'Failed to load ruby plugin host.' .
    \ " Try upgrading the Neovim ruby gem with 'gem install neovim-plugin-host'"
    \ " If you are still having trouble, try setting: "
    \ "'let g:ruby_host_prog=/path/to/ruby /path/to/start-neovim-ruby-plugin-host' in your '.nvimrc'"
endfunction

call remote#host#Register('ruby', function('s:RequireRubyHost'))
" }}}
